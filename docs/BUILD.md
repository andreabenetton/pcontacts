...
The build workflow (not the release environment) additionally uses the
repository secret `NVD_API_KEY`: CI's own free NVD API key for OWASP
Dependency-Check, see [§Vulnerability scanning](#vulnerability-scanning).
Nothing local needs a key.

The build workflow (not the release environment) additionally uses the
repository secret `NVD_API_KEY`: a free NVD API key for OWASP
Dependency-Check, see [§Vulnerability scanning](#vulnerability-scanning).
...
| Mechanism | Source | When | Gate |
|---|---|---|---|
| Audit snapshot (ADR-0024) | one osv.dev batch query, committed file | regenerated by the owner before a release | CI fails on classpath drift or on an open advisory the snapshot does not list |
| Runtime check (ADR-0025) | osv.dev, from the device | daily, only when the user switches it on | none: it informs the user |
| OWASP Dependency-Check | NVD mirror (plus CISA KEV, OSS Index) | CI: every push and pull request, Mondays 09:00 UTC, on demand | fails the scan job on an unsuppressed CVSS ≥ 7.0 |

### OWASP Dependency-Check

Configured in `app/build.gradle.kts` (`dependencyCheck { … }`) and run by
the `vulnerability-scan` job of `.github/workflows/build.yml`.

- **What it scans.** Only `releaseRuntimeClasspath` of `:app`: the
  artifacts that reach the APK. Test, build, lint and annotation-processor
  classpaths are out of scope on purpose (they pull gRPC, Netty, protobuf
  and the Kotlin compiler with their own CVE histories, none of which
  ships).
- **What fails.** Any finding with CVSS 7.0 or higher that is not covered
  by `config/dependency-check-suppressions.xml`. Lower scores are
  reported, not fatal.
- **Data.** Dependency-Check mirrors the NVD locally through the NVD API
  2.0, which needs the `NVD_API_KEY` repository secret (a free key from
  https://nvd.nist.gov/developers/request-an-api-key, activated through
  the emailed link). CI caches the mirror under
  `~/.gradle/dependency-check-data` between runs; the cache is restored
  and saved with `if: always()` so a timed-out download keeps its
  progress. Data is trusted for 7 days (`nvd.validForHours = 168`), which
  matches the weekly cadence.
- **Reports.** HTML, JSON and SARIF land in
  `app/build/reports/dependency-check/`; CI uploads them as the
  `dependency-check-reports` artifact and sends the SARIF to the GitHub
  Security tab (`category: dependency-check`). The JSON is also what
  `verifyDependencyAudit` compares with the shipped snapshot as a second
  opinion (open CVEs by id or alias).
- **Suppressions.** `config/dependency-check-suppressions.xml`. Every
  `<suppress>` entry needs a `<notes>` block saying why the finding does
  not apply and a re-evaluation date, and either a `<cve>` id or a
  `<packageUrl regex>` for the artifact. Rules:
  - Pin the exact artifact version in the package URL wherever the
    reasoning depends on how a specific version bundles or uses the code
    (the WorkManager inspector jar is the example). A later bump then
    re-opens the finding by design, so it gets assessed again.
  - Open the notes with `False positive:` only when the scanner matched a
    different product than the artifact (a CPE mismatch, the SQLite case).
    The notes are shown in the app: a false positive leaves the artifact
    green; any other suppression is a real advisory assessed as not
    applicable and shows the artifact as amber with these notes.
  - A suppression is never a way to silence a real, reachable
    vulnerability; the fix is a bump.
- **Running it locally.** Optional and slow: the first run mirrors the
  whole NVD (30 to 90 minutes, about 600 MB in
  `~/.gradle/dependency-check-data`), later runs fetch deltas.

  ```bash
  NVD_API_KEY=… ./gradlew :app:dependencyCheckAnalyze
  ```

  Nothing in the release process depends on a local run; the audit
  snapshot is generated from osv.dev and takes seconds.
- **Reading a failure.** The scan job's log names the artifact, the CVE
  and the score. Decide in this order: bump the dependency; if no fixed
  version exists and the code path is unreachable, write an assessed
  suppression with the reasoning; if the match is a CPE mismatch, write a
  `False positive:` suppression. Then regenerate the audit snapshot if the
  classpath changed.
...
- **What fails.** Any finding with CVSS 7.0 or higher that is not covered
  ...
  reported, not fatal.
- **Data.** Dependency-Check mirrors the NVD locally through the NVD API
  2.0, which needs the `NVD_API_KEY` repository secret (CI's own key). CI
  caches the mirror under `~/.gradle/dependency-check-data` between runs;
  the cache is restored and saved with `if: always()` so a timed-out
  download keeps its progress. Data is trusted for 7 days
  (`nvd.validForHours = 168`), which matches the weekly cadence.
- **Reports.** HTML, JSON and SARIF land in
  `app/build/reports/dependency-check/`; CI uploads them as the
  `dependency-check-reports` artifact and sends the SARIF to the GitHub
  Security tab (`category: dependency-check`). The JSON is also what
...
- **Suppressions.** `config/dependency-check-suppressions.xml`. Every
  ...
  - A suppression is never a way to silence a real, reachable
    vulnerability; the fix is a bump.
- **Running it locally.** Not needed for anything and slow: the first run
  mirrors the whole NVD (30 to 90 minutes, about 600 MB in
  `~/.gradle/dependency-check-data`). If you ever want it, put a key of
  your own in the environment:

  ```bash
  NVD_API_KEY=… ./gradlew :app:dependencyCheckAnalyze
  ```

  The audit snapshot is generated from osv.dev and takes seconds.
- **Reading a failure.** The scan job's log names the artifact, the CVE
  and the score. Decide in this order: bump the dependency; if no fixed
  version exists and the code path is unreachable, write an assessed
  suppression with the reasoning; if the match is a CPE mismatch, write a

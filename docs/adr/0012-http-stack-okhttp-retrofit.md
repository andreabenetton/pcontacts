...
- `ProtonCertificatePins` — `CertificatePinner` driven by `/proton_certificate_pins.txt` on the classpath. Resource file is committed with two ISRG root pins (Root X1 RSA + Root X2 EC), captured 2026-05-24 and cross-verified against the live `api.proton.me` chain. Root-level pinning survives leaf (~90-day) and intermediate rotations; breaks only if Proton migrates away from the ISRG CA entirely.

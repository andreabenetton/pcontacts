...
- The user can disable sync at any time from system settings; we honor that and stop initiating syncs (`ContentResolver.getSyncAutomatically()`). Amended 2026-09-22: only explicit user actions — "Sync now", sign-in, a linked-contact import — may carry `SYNC_EXTRAS_MANUAL` (which means `IGNORE_SETTINGS`); the periodic WorkManager fallback and the foreground refreshes never do, and they check the master and per-account switches before requesting (`SyncRequests.requestIfEnabled`). Amended 2026-09-22 (later): the per-account switch is also driven from inside the app — the sync-interval slider's first position, "Off", calls `ContentResolver.setSyncAutomatically(account, AUTHORITY, false)` and any cadence sets it back to `true` (the chosen cadence is kept while off); the slider reads the switch on every resume, so a change made in Android Settings shows as "Off" in the app. The phone-wide "Auto-sync data" switch is never changed by the app: when it is off the sync card says so and links to Android's sync settings, because only "Sync now" works then.

...
| Encrypted **merge base** (`contact_map.last_known_server_payload`, ADR-0017 §3 / ADR-0018) | until the next pull or push overwrites it, or logout | AES-256-GCM (IV‖ct‖tag) under the Keystore AEAD KEK `pcontacts.kekv1`, stored inside the plaintext SQLite file; sealed before the row is written; photo bytes excluded (two SHA-256 digests of the photo stand in for it) | never on the wire | Without the KEK: ciphertext only. With the KEK (same-UID compromise): the last-synced contact fields of every contact, excluding photos — the same exposure as the ContactsContract rows the app already writes. |
| **Outbox** (`outbox` table in Room) | until push succeeds or is discarded; one live row per contact | plaintext SQLite; stores `op_type`, `payload_hash`, attempt metadata — no decrypted content | never on the wire | Contact IDs + operation types (low sensitivity). |
...
| S4 | A malicious app dispatches `ACTION_VIEW` with an arbitrary `content://` URI at the exported `SendViaProtonMailActivity`, making it read a foreign row under our `READ_CONTACTS` or open an attacker-chosen `mailto:`. | The activity acts only on a ContactsContract Data row of our MIME type whose RawContact belongs to the pcontacts account (authority, path, provider-reported type and account checked before any read), builds the `mailto:` with `Uri.fromParts`, and finishes quietly otherwise. Covered by `SendViaProtonMailActivityTest`. | Low. |
...
| T5 | The server substitutes an address key's `Token` (or its `Signature`) so that a key it controls unlocks and joins the verification-key set; cards it signs would then read as verified. | The Token's detached `Signature` must verify under the user's own keys before the address key is used for anything; an unsigned or wrongly signed Token skips the key entirely (ADR-0020 amendment). Legacy v1 keys unlock with `keyPassword`, the trust anchor itself. | Low — closes the `[D]` deferral; residual: the user primary key is still the root of trust, as it must be. |
...
| D2 | Server returns Code 9001 (human-verification challenge) on every call. | `HumanVerificationInterceptor` throws `HumanVerificationRequiredException` with the verification URL `[U]` extracted from the response body; SyncAdapter maps to `numAuthExceptions` so the sync framework stops retrying. `VerificationNotifier` posts a high-priority notification; tapping it launches `MainActivity`, which opens the captcha page in the locked-down in-app `WebView` of `HumanVerificationActivity` (ADR-0019) via `HumanVerificationLauncher`; the page's success envelope stores the verification token in `SecretStore`, and on return `onResume` fires a one-shot expedited sync (honouring the sync switches). During sign-in, a 9001 on `/auth` or `/auth/2fa` routes to the same WebView from `LoginActivity`. If the URL is null, a fallback dialog directs the user to complete verification in a browser manually. | Low — in-app recovery path now exists; residual: the verification URL shape is `[U]` and may change without notice. |

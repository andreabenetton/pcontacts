...
**v1.3.0 released.** **Validated against the live Proton production API** — full SRP handshake, token persistence, keyPassword derivation, multi-key contact decrypt (user + address keys), and logout all succeed. See [`docs/API_RESEARCH.md`](docs/API_RESEARCH.md) for protocol details.
...
2. **`x-pm-appversion` window drift.** The hardcoded version (`android-mail@7.10.4`) will age out as Proton releases updates. Requires periodic maintenance bumps.

...
- **The dependency audit ships in the app.** A chip next to the version says whether any shipped artifact has a known CVE ("Dependencies OK"; assessed CVEs, amber; vulnerability, red) and a Dependencies screen lists every artifact with license and CVE links. It is a build-time snapshot committed to the repo, verified by CI against the resolved classpath and the weekly scan; a scanner false positive is listed but does not colour the artifact. (ADR 0024)
- **Optional runtime check, off by default.** A switch in Privacy sends the same artifact list (public in this repo) once a day to osv.dev, which then sees the device's address; new advisories turn the chip red and are announced once, and the user can mute one, which counts as assessed until the artifact changes version. Nothing else in the app depends on the answer. (ADR 0025)
